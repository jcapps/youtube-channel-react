...
export class VideosRemovedFromPlaylistsPage extends React.PureComponent {
    ...
    showLoadingSpinner() {
        $('#videos-removed-from-playlists-page .loading-spinner').removeClass('hidden');
    }

    hideLoadingSpinner() {
        $('#videos-removed-from-playlists-page .loading-spinner').addClass('hidden');
    }
    ...
    getData(state) {
        ...
        if (this.props.graphType == GraphTypes.GEO) {
            const sort = '-' + Metrics.VIDEOS_REMOVED_FROM_PLAYLISTS.metric;
    ...
    renderGraphContainer() {
        if (this.state.playlistAttempted)
            return <div className="error-message">The videos removed from playlists metric does not allow filtering by playlist.</div>;
        if (!this.props.videosRemovedFromPlaylists.columnHeaders) return <div/>;
...
VideosRemovedFromPlaylistsPage.propTypes = {
    isLoading: PropTypes.bool.isRequired,
    videosRemovedFromPlaylists: PropTypes.object.isRequired,
...
export function mapStateToProps(state) {
    ...
    return {
        videosRemovedFromPlaylists: state.report,
...
export const connectOptions = {
    areStatePropsEqual: (next, prev) => {
        return !(
            (!next.isLoading) || 
            ((prev.videosRemovedFromPlaylists !== next.videosRemovedFromPlaylists) && (prev.totalStats !== next.totalStats))
        );
    }
};

export default connect(mapStateToProps, mapDispatchToProps, null, connectOptions)(VideosRemovedFromPlaylistsPage);