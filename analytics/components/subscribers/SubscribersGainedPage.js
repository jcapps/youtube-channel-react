...
export class SubscribersGainedPage extends React.PureComponent {
    ...
    componentDidMount() {
        document.title = `Analytics: ${Metrics.SUBSCRIBERS_GAINED.displayName}`;
        window.scrollTo(0, 0);
    ...
    getData(state) {
        ...
        if (this.props.graphType == GraphTypes.GEO) {
            const sort = '-' + Metrics.SUBSCRIBERS_GAINED.metric;
    ...
    render() {
        ...
        return (
            <div id="subscribers-gained-page">
                <h2>{Metrics.SUBSCRIBERS_GAINED.displayName}</h2>
                <FiltersSection
