...
export class CardTeaserClickRatePage extends React.PureComponent {
    ...
    componentDidMount() {
        document.title = `Analytics: ${Metrics.CARD_TEASER_CLICK_RATE.displayName}`;
        window.scrollTo(0, 0);
    ...
    showLoadingSpinner() {
        $('#card-teaser-click-rate-page .loading-spinner').removeClass('hidden');
    }

    hideLoadingSpinner() {
        $('#card-teaser-click-rate-page .loading-spinner').addClass('hidden');
    }
    ...
    render() {
        ...
        return (
            <div id="card-teaser-click-rate-page">
                <h2>{Metrics.CARD_TEASER_CLICK_RATE.displayName}</h2>
                <FiltersSection
...
CardTeaserClickRatePage.propTypes = {
    isLoading: PropTypes.bool.isRequired,
    cardTeaserClickRate: PropTypes.object.isRequired,
...
export const connectOptions = {
    areStatePropsEqual: (next, prev) => {
        return !(
            (!next.isLoading) || 
            ((prev.cardTeaserClickRate !== next.cardTeaserClickRate) && (prev.totalStats !== next.totalStats))
        );
    }
};

export default connect(mapStateToProps, mapDispatchToProps, null, connectOptions)(CardTeaserClickRatePage);